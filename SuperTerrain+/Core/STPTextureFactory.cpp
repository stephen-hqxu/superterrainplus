...
STPTextureFactory::STPTextureFactory(const STPTextureDatabase& database) {
	//we build the data structure that holds all texture in groups first
	//get the data
	const STPTextureDatabase::STPGroupRecord group = database.getValidGroup();
	const STPTextureDatabase::STPTextureCollectionRecord collection = database.getValidTexture();
	const STPTextureDatabase::STPTextureDataRecord data = database.getValidTextureData();
	...
	this->TextureRegionLookup.resize(database.textureCollectionSize() * static_cast<TEXTYPE_TYPE>(STPTextureType::TypeCount), numeric_limits<unsigned int>::max());
	...
	for (const auto [group_id, texture_id, type, img] : data) {
		...
		const TEXTYPE_TYPE type_idx = static_cast<TEXTYPE_TYPE>(type);
		...
		this->TextureRegionLookup[texture_idx * static_cast<TEXTYPE_TYPE>(STPTextureType::TypeCount) + type_idx] = 
