...
template<typename T>
inline SuperTerrainPlus::STPRealism::STPShaderManager::STPMacroValueDictionary& 
	SuperTerrainPlus::STPRealism::STPShaderManager::STPMacroValueDictionary::operator()(const std::string& macro, T value) {
	...
	if constexpr (disjunction_v<
		...
		this->Macro.try_emplace(macro, value);
	}
	else {
		//value is not a string, needs to be converted first
		this->Macro.try_emplace(macro, std::to_string(value));
	}
	return *this;
}
