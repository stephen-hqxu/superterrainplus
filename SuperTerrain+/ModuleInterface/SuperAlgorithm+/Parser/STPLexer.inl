...
LEXER_TEMPLATE
inline NAMESPACE_LEXER_NAME::STPLexer::STPToken::STPToken() noexcept : TokenID(STPLexerToken::NullTokenID) {

}

LEXER_TEMPLATE
inline NAMESPACE_LEXER_NAME::STPLexer::STPToken::STPToken(const STPLexerToken::STPTokenID id, const char* const beg, const size_t count) :
	TokenID(id), Lexeme(beg, count) {
...
LEXER_TEMPLATE
template<class Tok>
inline bool NAMESPACE_LEXER_NAME::STPLexer::STPToken::operator==(const Tok&) const noexcept {
...
LEXER_TEMPLATE
inline NAMESPACE_LEXER_NAME::STPLexer(const std::string_view& input, const std::string_view& lexer_name, const std::string_view& source_name) noexcept :
	Sequence(input.data()), Line(1u), Character(1u), LexerName(lexer_name), SourceName(source_name) {
...
LEXER_TEMPLATE
inline typename NAMESPACE_LEXER_NAME::STPToken NAMESPACE_LEXER_NAME::createToken(const STPLexerToken::STPTokenID id, const size_t length) {
	const char* const character = this->Sequence;
	//we can safely move the pointer forward because the pointer in the character is not owned by the view
	this->pop(length);
	return STPToken(id, character, length);
}

LEXER_TEMPLATE
inline void NAMESPACE_LEXER_NAME::correctStats(const STPToken& token) {
...
LEXER_TEMPLATE
inline typename NAMESPACE_LEXER_NAME::STPToken NAMESPACE_LEXER_NAME::nextAtomicToken() {
	...
	if (((nextID = AtomToken::ID, nextAtom == AtomToken::Representation) || ...)) {
		//found
...
inline typename NAMESPACE_LEXER_NAME::STPToken NAMESPACE_LEXER_NAME::nextFunctionToken() {
